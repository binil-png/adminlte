...
function dateFilter(data) {
  ...
  $("#dateFilter").select2({
    ...
    selectionCssClass:
      "form-control rounded-4 input-style custom-select ignore-edit",
    data,
  });
}
...
async function applyFilters() {
  ...
  container.empty();
  let api = new SidebarServices(container);
  ...
  if (activeFilter == "vitals") {
    ...
    dateFilter(
      apiVitalsData?.results?.vitals.map((i) => ({ id: i.date, text: i.date })),
    );
  ...
  if (activeFilter == "notes") {
    ...
    dateFilter(
      apiNotessData?.results.map((i) => ({ id: i.date, text: i.date })),
    );
  ...
  if (activeFilter == "prescription") {
    ...
    dateFilter(
      apiPrescriptionData?.results.map((i) => ({ id: i.date, text: i.date })),
    );
  ...
  if (activeFilter == "procedure") {
    ...
    dateFilter(
      apiProcedureData?.results.map((i) => ({ id: i.date, text: i.date })),
    );
